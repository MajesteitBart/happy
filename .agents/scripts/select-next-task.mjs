...
const requestedProject = readOption("--project") || "delano-multi-agent-execution";
const projectSlug = resolveProjectSlug(requestedProject);
...
function resolveProjectSlug(slug){
  const projectDir = path.join(repoRoot, ".project", "projects", slug);
  if (existsSync(path.join(projectDir, "tasks"))) return slug;
  const projectsRoot = path.join(repoRoot, ".project", "projects");
  const fallback = existsSync(projectsRoot)
    ? readdirSync(projectsRoot).find((entry)=>existsSync(path.join(projectsRoot, entry, "tasks")))
    : "";
