...
function runSelfTest() {
  ...
  const leaseManager = resolveScript("lease-manager.mjs");
  mkdirSync(dir, { recursive: true });
  try {
    const acquire = spawnSync(process.execPath, [leaseManager, "acquire", "--state", state, "--owner", "handoff-test", "--project", "delano-multi-agent-execution", "--task", "T-006", "--zone", ".agents/scripts/lease-manager.mjs"], { cwd: repoRoot, encoding: "utf8" });
...
function resolveScript(name) {
  const candidates = [path.join(repoRoot, ".agents", "scripts", name), path.join(repoRoot, "scripts", name)];
